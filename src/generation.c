...
void genTrajectories(BallMotion* ballMotions, int nBalls)
{
    ...
    for (i = 0; i < nBalls; i++)
    {
        ...
        if (i == 0) ballMotions[i].traj = (Vector2){ -50, 0 };
        else if (i == 1) ballMotions[i].traj = (Vector2){ -50 * cosf(PI/4), -50 * sinf(PI/4) };
    }
}
...
void genLocations(BallMotion* ballMotions, int nBalls, int radius)
{
    ...
    for (i = 0; i < nBalls; i++)
    {
        ...
        if (i == 0) ballMotions[i].pos = (Vector2){ 200, 50 };
        else if (i == 1) ballMotions[i].pos = ballMotions[i].pos = (Vector2){ 50 + 150 * cosf(PI/4), 50 + 150 * sinf(PI/4) };
    }
}